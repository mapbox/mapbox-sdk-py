...
class Directions(Service):
    ...
    valid_profiles = [
        "mapbox/driving", "mapbox/driving-traffic", "mapbox/walking", "mapbox/cycling"
    ]
    ...
    @property
    def baseuri(self):
        return "https://{0}/{1}/{2}".format(self.host, self.api_name, self.api_version)
    ...
    def _validate_profile(self, profile):
        ...
        if profile in v4_to_v5_profiles:
            ...
            warnings.warn(
                "Converting v4 profile to v5, use {} instead".format(profile),
    ...
    def _validate_geom_encoding(self, geom_encoding):
        if geom_encoding is not None and geom_encoding not in self.valid_geom_encoding:
            raise errors.InvalidParameterError(
    ...
    def directions(
        ...
        **kwargs
        ...
        if "geometry" in kwargs and geometries is None:
            ...
            warnings.warn(
                "Use `geometries` instead of `geometry`",
                errors.MapboxDeprecationWarning,
            )
        ...
        uri = URITemplate(
            self.baseuri + "/{profile_ns}/{profile_name}/{coordinates}.json"
        ).expand(
