...
def Session(access_token=None, env=None):
    ...
    if env is None:
        env = os.environ.copy()
    access_token = (
        access_token or env.get("MapboxAccessToken") or env.get("MAPBOX_ACCESS_TOKEN")
    )
...
class Service(object):
    ...
    def username(self):
        ...
        if not token:
            raise errors.TokenError("session does not have a valid access_token param")
    ...
    def handle_http_error(self, response, custom_messages=None, raise_for_status=False):
