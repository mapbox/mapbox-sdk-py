...
def validate_lat(val):
    if val < -85.0511 or val > 85.0511:
        raise errors.InvalidCoordError("Latitude must be between -85.0511 and 85.0511")
...
def validate_lon(val):
    if val < -180 or val > 180:
        raise errors.InvalidCoordError("Longitude must be between -180 and 180")
...
def validate_image_size(val):
    if not 1 < val < 1280:
        raise errors.ImageSizeError("Image height and width must be between 1 and 1280")
...
class StaticStyle(Service):
    ...
    def tile(self, username, style_id, z, x, y, tile_size=512, retina=False):
        ...
        values = dict(
            username=username, style_id=style_id, tile_size=tile_size, z=z, x=x, y=y
        )
    ...
    def image(
        ...
        if retina is None:
            if twox is not None:
                warnings.warn(
                    "twox is a deprecated alias for retina",
        ...
        if features:
            ...
            if auto:
                raise errors.InvalidCoordError("Must provide features or lat, lon, z")
